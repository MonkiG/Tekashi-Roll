...
import { type Product, type AddProduct, type Category } from '@/app/types'
...
export default function ProductModal ({ handleCloseModal, product }: { handleCloseModal: () => void, product?: AddProduct | Product }): JSX.Element {
  const [categories, setCategories] = useState<Category[] | null>()
  const initial: AddProduct = {
    name: '',
    price: '',
    description: '',
    categoryId: '',
  ...
  const [productData, setProductData] = useState<AddProduct | Product>(product ?? initial)
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    if (product) {
      ...
      await editProductById((product as Product).id, rest)
      toast.success('Producto editato correctamente!')
    } else {
      await addProduct(productData)
      toast.success('Producto arreglado correctamente!')
    }
    handleCloseModal()
    handleAgain()
  }
