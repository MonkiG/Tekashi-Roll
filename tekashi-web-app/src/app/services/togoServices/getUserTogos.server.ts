...
export async function getUserTogos (userId: UUID): Promise<any> {
  ...
  const { data: togos } = await supabase.from('product_togo').select('togo(total, id, status), products(*, categories(name))').eq('togo.user_id', userId).not('togo.status', 'eq', 'delivered')
